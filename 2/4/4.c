...
double polynomial_value(double x, int n, ...) {
    if (n < 0) {
        printf("Invalid polynomial degree. Must be non-negative.\n");
        return -1;
    }
...
int is_kaprekar(int num, int base) {
    if (num < 0 || base < 2 || base > 36) {
        printf("Invalid base or number. Base must be between 2 and 36; number must be non-negative.\n");
        return 0;
    }
...
void find_kaprekar_numbers(int base, int count, ...) {
    if (base < 2 || base > 36) {
        printf("Invalid base. Must be between 2 and 36.\n");
        return;
    }
...
int main() {
    ...
    check_of_convex(4, 0.0, 0.0, 2.0, 1.0);
    check_of_convex(10, 0.0, 0.0, 2.0, 1.0, 1.0, 2.0, 0.5, 1.5, 1.5, 0.5);

    printf("\n=== Polynomial Evaluation ===\n");
    polynomial_value(2.0, -2, 3.0, -2.0, 1.0); // 3x^2 - 2x + 1, x = 2.0
